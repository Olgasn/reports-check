...
import { LabLink } from './course-results.styled';

export const CourseResults: FC = () => {
  ...
  return (
    <Box display="flex" flexDirection="column">
      <TopHeader
        ...
        subText={'Здесь вы можете быстро найти результаты проверок по лабораторным работам.'}
      />
      ...
      <Box display="flex" flexDirection="column" gap={1}>
        {labs.map(({ id, name }) => (
          <Card sx={{ px: 2, py: 1 }}>
            <LabLink to={`/labs/${id}/checks`}>{name}</LabLink>
          </Card>
        ))}
      </Box>
    </Box>
